```dataviewjs
/********************************************************************
* Planner_6_10 — Resume Menu Bugfix
********************************************************************/
...
let EMBED_VIEW_PATH = PRIMARY_AGENDA_FILE; 
                            ...
                            if (parts.length >= 5) {
                                ...
                                if(id) {
                                    ...
                                    if (s === "DELETED" || e === "DELETED") {
                                        // If local file says deleted, remove from map
...
.embed-box { display:none; position:relative; width:100%; height:500px; background:var(--background-primary); border:1px solid var(--background-modifier-border); border-radius:8px; overflow:auto; margin-top:8px; padding:20px; }
...
function setPlannerDate(dStr){
  ...
  EMBED_VIEW_PATH = PRIMARY_AGENDA_FILE;
...
async function EndActivity(activityName, endTime, preciseUnix = null, targetFile = null){
  try {
      const unifiedMap = await GetUnifiedActivityMap();
      let activity = null;
...
async function EditActivity(rowId, field, newVal, preciseUnix = null, targetFile) {
  try {
      const unifiedMap = await GetUnifiedActivityMap();
...
async function DeleteActivity(rowId, activityName, targetFile) {
...
async function HandleBreak() {
    ...
    if (isOnBreak) {
        ...
        for (const brk of activeBreaks) {
             const breakStartUnix = await EndActivity("Break", now, ts, brk.sourceFile);
             const didPrompt = await MaybePromptResumeFromUnix(breakStartUnix, ["Break"]);
             if (!didPrompt) task.textContent = "Break ended.";
        }
    } else {
        if (active.length > 0) for (const act of active) await EndActivity(act.name, now, ts, act.sourceFile);
...
async function RefreshEmbed(){
    embedBox.innerHTML = "";
    const file = app.vault.getAbstractFileByPath(PRIMARY_AGENDA_FILE);
    if(file) await renderMd(await app.vault.read(file), embedBox, PRIMARY_AGENDA_FILE, COMPONENT);
}
...
function selectItem(data) {
    ...
    if (suggestMode === 'agenda-root') { pendingAction = data.code; suggestMode = 'agenda-select-activity'; text_box.value = ''; rerender(); }
        ...
        (async () => {
            ...
            if (data.choice === 'overlap_yes') for (const act of payload.active) await EndActivity(act.name, payload.startTime, payload.preciseUnix, act.sourceFile);
